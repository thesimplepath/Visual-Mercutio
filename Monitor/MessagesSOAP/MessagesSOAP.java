...
public class MessagesSOAP
{
  ...
  /**
  * Allow to insert n messages (with n deliverables) into the database.
  ...
  */
  ...
  {
    NodeList listOfElements, listOfAttributes;
    Node element;
    String ExternDelivID, ExternUserID, TimestampDeliv, DoubleControl;
    int message_id;
    ...
    try
    {
      ...
      /***** message ******/
      listOfElements = doc.getElementsByTagName("message");

      for (int j = 0; j < listOfElements.item(i); j++)
      {
        NamedNodeMap attr = listOfElements.item(i).getAttributes();
        message_id = this.addMessage(getAttributeText(attr, "externmessageid"), getAttributeText(attr, "externsystemid"), getAttributeText(attr, "externprocid"), getAttributeText(attr, "externuserid"), getAttributeText(attr, "timestampproc"), getAttributeText(attr, "messagetime"), getAttributeText(attr, "statut"), getAttributeText(attr, "caseid"), getAttributeText(attr, "caseidremark"));

        /***** message deliv *****/
        listOfElements = listOfElements.item(0).getChildNodes();
        ...
        for (int i = 0; i < listOfElements.getLength(); i++)
        {
          ...
          {
            ExternDelivID = element.getAttributes().getNamedItem("externdelivid").getNodeValue();
            ...
            this.addDeliv(message_id, ExternDelivID, ExternUserID, TimestampDeliv, DoubleControl);
          }
        }
  ...
  * @throws Exception
  */
  protected int addMessage(String ExternMessageID, String ExternSystemID, String ExternProcID, String ExternUserID, String TimestampProc, String MessageTime, String Statut, String CaseID, String CaseIDRemark) throws Exception
  {
    ...
    System.out.println("Message : " + ExternMessageID + " " + ExternSystemID + " " + ExternProcID + " " + ExternUserID + " " + TimestampProc + " " + MessageTime + " " + Statut + " " + CaseID + " " + CaseIDRemark);
    ...
    // ****************** insert the message
    ps = _conn.prepareStatement("insert into MESSAGE(ExternMessageID,ExternSystemID,ExternProcID,ExternUserID,TimeStampProc,MessageTime,Statut,CaseID,CaseIDRemark,MessageErrorID) values(?,?,?,?,?,?,?,?,?,null)");
  ...
  * @throws Exception
  */
  protected void addDeliv(int IdMessageInterne, String ExternDelivID, String ExternUserID, String TimestampDeliv, String DoubleControl) throws Exception
  {
    PreparedStatement ps;
    ...
    System.out.println("Deliv : " + IdMessageInterne + " " +  ExternDelivID + " " +  ExternUserID + " " +  TimestampDeliv + " " +  DoubleControl);
    ...
    // ***** check params
    if (l_DoubleControl < 1 || l_DoubleControl > 3)
    {
    ...
    // insert
    ps = _conn.prepareStatement("insert into MESSAGEDELIV(IdMessageInterne,ExternDelivID,ExternUserID,TimestampDeliv,DoubleControl,MessageErrorID) values(?,?,?,?,?,null)");
  ...
  protected void loadConfig()
  {
    ...
    try
    {
      ...
      if (f.exists())
      {
        ...
        // todo récupérer le host depuis le fichier avec les alians
      ...
      {
        System.out.println("config file (configMessagesSOAP.ini) not found. Default values have been loaded.");
      }
