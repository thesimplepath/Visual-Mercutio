...
<%
 ...
 ars=aconn.sqlQueryR("SELECT * FROM pproc,pprocattr WHERE pprocattr.procattribid=pproc.procattribid AND pprocattr.prow=0 AND pproc.procid="+Integer.valueOf(request.getParameter( "PROCID" )).intValue());
 ars.next();
...
<%
 ...
 rs=conn.sqlQueryR("SELECT pprocattr.attribdefid as attribdefid,pprocattr.mvalue as mvalue FROM pproc,pprocattr WHERE pproc.procid="+procid+" AND pproc.procattribid=pprocattr.procattribid AND pprocattr.attribdefid>=13 AND pprocattr.attribdefid<=32");
...
<%
 ...
 conn=PsDbMgr.newConnection();
 rs=conn.sqlQueryR("SELECT pprocattr.attribdefid as attribdefid,pprocattr.mvalue as mvalue FROM pproc,pprocattr WHERE pproc.procid="+procid+" AND pproc.procattribid=pprocattr.procattribid AND pprocattr.attribdefid>=33 AND pprocattr.attribdefid<=52");
...
<%
 ...
 rs=conn.sqlQueryR("SELECT pprocattr.attribdefid as attribdefid,pprocattr.mvalue as mvalue FROM pproc,pprocattr WHERE pproc.procid="+procid+" AND pproc.procattribid=pprocattr.procattribid AND pprocattr.attribdefid in (9,10)");
...
//		return;
 ...
 while (rs.next())
 {
	...
	conn2=PsDbMgr.newConnection();
	rs2=conn2.sqlQueryR("SELECT * FROM pproc,pprocattr WHERE pproc.procid="+proctrg+" and pproc.procattribid=pprocattr.procattribid and pprocattr.prow=0;");
	rs2.next();	
