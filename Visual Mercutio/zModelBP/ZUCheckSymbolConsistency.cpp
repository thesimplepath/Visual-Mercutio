...
bool ZUCheckSymbolConsistency::CheckDoorSymbol(PSS_DoorSymbolBP* pSymbol)
{
    ASSERT(pSymbol);
...
bool ZUCheckSymbolConsistency::CheckProcedureSymbol(PSS_ProcedureSymbolBP* pSymbol)
{
    ...
    if (DownSymbolCount > 0 &&
        PSS_ODSymbolManipulator::HasSymbolsISA(DownNodes, RUNTIME_CLASS(PSS_ProcessSymbolBP)))
    {
        if (m_pLog)
        {
            PSS_GenericSymbolErrorLine e(IDS_EL_PROCEDURE_INC_3,
...
bool ZUCheckSymbolConsistency::CheckProcessSymbol(PSS_ProcessSymbolBP* pSymbol)
{
    ASSERT(pSymbol);
...
bool ZUCheckSymbolConsistency::CheckStopSymbol(PSS_StopSymbolBP* pSymbol)
{
    ASSERT(pSymbol);
