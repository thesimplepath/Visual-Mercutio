...
//---------------------------------------------------------------------------
bool PSS_StartSymbolBP::ProcessMenuCommand(int menuCmdID, ZBProperty& prop, CString& value, ZBPropertySet& props, bool& refresh)
{
    const int categoryID = prop.GetCategoryID();
...
CString PSS_StartSymbolBP::GetRuleNameByGUID(PSS_LogicalRulesEntity* pRule, const CString& ruleGUID)
{
    ...
    if (pRule->ContainEntity())
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            PSS_LogicalRulesEntity* pRuleEntity = dynamic_cast<PSS_LogicalRulesEntity*>(pEntity);
