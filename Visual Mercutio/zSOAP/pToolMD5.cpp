...
{
    if ( nEncryptLength<1 || nEncryptLength>( 2 * MD5_HASH_LENGTH ) )
    {
        printf( _T( "md5encode: out of boundaries...\n" ) );
        return data;
    }
    ...
    char                result[2 * MD5_HASH_LENGTH + 1];
    int                    i;

    MD5Init( &md );
    MD5Update( (MD5_CTX*)&md, (unsigned char*)data.c_str(), data.length() );
    MD5Final( sum, &md );
