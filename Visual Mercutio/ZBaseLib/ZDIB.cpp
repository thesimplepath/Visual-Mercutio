...
ZDIB::ZDIB(HANDLE hDib,int nBits)
{
	m_pVoid = NULL;
	m_pLinePtr = NULL;
	m_bUseGamma=FALSE;
...
BOOL ZDIB::Create(int width,int height,int bits)
{
	/*
		Free existing image
	*/
...
BOOL ZDIB::Create(BITMAPINFOHEADER& bmInfo)
{
...
void ZDIB::SetPalette(unsigned char *palette)
{
int i,size;
RGBQUAD *rgb;
	if(!palette) return;
...
void ZDIB::SetPalette(RGBQUAD *pRGB)
{
int size;
	if(!pRGB) return;
	size = GetPaletteSize();
...
int ZDIB::GetPaletteSize()
{
	return GetPaletteSize(m_pInfo->bmiHeader);
}


int ZDIB::GetPaletteSize(BITMAPINFOHEADER& bmInfo)
{
...
void ZDIB::SetPixel(int x,int y,COLORREF color)
{
unsigned char *ptr;
	ASSERT(x >= 0 && y >=0);
...
void ZDIB::SetPixel8(int x,int y,unsigned char color)
{
...
COLORREF ZDIB::GetPixel(int x,int y)
{
unsigned char *ptr;
COLORREF color;
	ASSERT(x >= 0 && y >=0);
...
CBitmap *ZDIB::GetTempBitmap(CDC& dc)
{
HBITMAP hBitmap;
CBitmap *temp;
	ASSERT(m_pVoid != NULL);
...
void ZDIB::CopyLine(int source,int dest)
{
...
void ZDIB::InitDIB(COLORREF color)
{
int i,j;
unsigned char *ptr;
...
void ZDIB::BitBlt(HDC hDest,int nXDest,int nYDest,int nWidth,int nHeight,int xSrc,int ySrc)
{
	SetDIBitsToDevice(hDest,nXDest,nYDest,nWidth,nHeight,xSrc,Height()-ySrc-nHeight,0,Height(),m_pBits,m_pInfo,DIB_RGB_COLORS);
}

void ZDIB::StretchBlt(HDC hDest,int nXDest,int nYDest,int nDWidth,int nDHeight,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
...
void ZDIB::ExpandBlt(int nXDest,int nYDest,int xRatio,int yRatio,ZDIB& dibSrc,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
...
void ZDIB::Expand(int nXDest,int nYDest,int xRatio,int yRatio,ZDIB& dibSrc,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
...
void ZDIB::StretchBlt(int nXDest,int nYDest,int nDWidth,int nDHeight,ZDIB& dibSrc,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
	SetPalette(dibSrc.m_pRGB);
	nDWidth = nXDest+nDWidth > width ? width-nXDest : nDWidth ; 
	nDHeight = nYDest+nDHeight > height ? height-nYDest : nDHeight;
...
void ZDIB::BitBlt(int nXDest,int nYDest,int nWidth,int nHeight,ZDIB& dibSrc,int nSrcX,int nSrcY,BYTE *colors)
{
	SetPalette(dibSrc.m_pRGB);
	if(nXDest < 0)
	{
...
unsigned char *ZDIB::GetLinePtr(int line)
{
/*unsigned char *ptr;
...
void ZDIB::ReplaceColor(unsigned char oldColor,unsigned char newColor)
{
int i,j;
unsigned char *ptr;	
	for(i=0; i < height; i++)
...
ZDIB& ZDIB::operator=(ZDIB& dib)
{
	CopyDIB(dib);
	return *this;
}

HANDLE ZDIB::GetDIBits(int nStartX,int nStartY,int nCx,int nCy)
{
	if(nStartX == -1)
	{
		nStartX = nStartY=0;
...
BOOL ZDIB::CreateFromHandle(HANDLE hMem,int bits)
{
	DestroyDIB();
UCHAR *lpVoid,*pBits;
LPBITMAPINFOHEADER pHead;
...
void ZDIB::UseGamma(float fg,BOOL bUse)
{
	m_bUseGamma = bUse;
...
void ZDIB::GetPixel(UINT x,UINT y,int& pixel)
{
	ASSERT(x < (UINT)Width());
...
/*
BOOL ZDIB::SwitchFrom24(ZDIB& dib)
{
int i,j,w,h;
...
BOOL ZDIB::SwitchFromOne(ZDIB& dib)
{
int i,j,w,h;
unsigned char *sPtr,*dPtr;
...
BOOL ZDIB::SwitchFromFour(ZDIB& dib)
{
int i,n,j,w,h;
unsigned char *sPtr,*dPtr;
...
BOOL ZDIB::SwitchPalette(ZDIB& dib)
{
int i,j,w,h;
unsigned char *sPtr,*dPtr;
...
int ZDIB::ClosestColor(RGBQUAD *pRgb)
{
...
unsigned int ZDIB::Distance(RGBQUAD& rgb1,RGBQUAD& rgb2)
{
unsigned int d;
...
BOOL ZDIB::OpenDIB(const CString& csFileName)
{
...
BOOL ZDIB::SaveDIB(CString& csFileName,BitmapType type)
{
	return SaveBMP(csFileName);
}

BOOL ZDIB::SaveBMP(CString& csFileName)
{
...
BOOL ZDIB::OpenBMP(const CString& csFileName)
{
BITMAPFILEHEADER bFile;
...
int ZDIB::EnumColors(BYTE *array)
{
	ASSERT(GetBitCount()==8);
BYTE *ptr;
int nNum=0,i,j,w,d;
...
COLORREF ZDIB::PaletteColor(int nIndex)
{
	ASSERT(nIndex < 256);
