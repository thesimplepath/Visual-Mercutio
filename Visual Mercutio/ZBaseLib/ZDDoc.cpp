...
PlanFinObject* ZDDocument::GetObjectPtr(CString& strName, int DocumentIndex)
{
    ...
    {
        ASSERT(GetActiveDocumentData());
        return GetActiveDocumentData()->GetObject(strName);
    }
...
BOOL ZDDocument::InsertExternalDocumentAfter(const CString    FileName,
                                             ...
                                             int            PropagateValue)
{
    PSS_DocumentData* pNewDocData = new PSS_DocumentData;
...
BOOL ZDDocument::DeleteDocumentData(PSS_DocumentData* pDocData)
{
    int Idx = GetDocumentDataIndex(pDocData);
    return (Idx != -1) ? DeleteDocumentAt(Idx) : FALSE;
}

int ZDDocument::GetDocumentDataIndex(PSS_DocumentData* pDocData)
{
    if (!pDocData)
    {
        return -1;
    ...
    {
        PSS_DocumentData* pData = GetDocumentDataAt(Idx);
