...
void ZIView::Initialize()
{
    ...
    CDC*    pDC = GetDC();
    ASSERT( pDC );
    ZBPaintOperations::SetLogicalBasicUnits( CSize( pDC->GetDeviceCaps( LOGPIXELSX ),
                                             pDC->GetDeviceCaps( LOGPIXELSY ) ) );

    m_iLogUnitX = ZBPaintOperations::GetLogicalBasicUnits().cx;
    m_iLogUnitY = ZBPaintOperations::GetLogicalBasicUnits().cy;
...
void ZIView::DrawPageRect ( CDC* pDC )
{
    ...
    CBrush*    pOldBrush = pDC->SelectObject( &ZBPaintResources::GetWhiteBrush() );
      CPen*    pOldPen = pDC->SelectObject( &ZBPaintResources::GetBlackPen() );

    pDC->Rectangle( &PageRect );
    pDC->SelectObject( &ZBPaintResources::GetGrayPen() );
