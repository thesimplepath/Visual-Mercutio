...
BEGIN_MESSAGE_MAP(ZCDatabaseListCtrl, PSS_ListCtrl)
...
ZCDatabaseListCtrl::ZCDatabaseListCtrl(const CString DatabaseFilename, const CString Tablename) :
    PSS_ListCtrl(), m_DatabaseFilename(DatabaseFilename), m_Tablename(Tablename), m_ColumnsBuilt(FALSE)
{
    m_pDataBase = NULL;
    m_pRecordSet = NULL;
}
...
void ZCDatabaseListCtrl::OnGetDispInfo(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    if (pDispInfo->item.mask & LVIF_TEXT)
    {
        ...
        switch (variant->vt)
        {
            ...
            case VT_BSTR:
            {
                CString str = V_BSTRT(&varValue);//convert BSTR to CString
            ...
            case VT_UI1:
            {
                wsprintf(szValue, "%d", variant->bVal);
                break;
            }
