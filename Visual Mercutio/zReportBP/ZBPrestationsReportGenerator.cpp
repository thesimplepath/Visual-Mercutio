...
void _ZBPrestationsDatas::GenerateHierarchy( ZBLogicalPrestationsEntity* pPrestations, int Level /*= 0*/ )
{
    ...
    if ( pPrestations->ContainEntity() )
    {
        ...
        for ( int i = 0; i < Count; ++i )
        {
            ...
            if ( ISA( pEntity, ZBLogicalPrestationsEntity ) )
            {
                ...
                m_ChildPrestation->GenerateHierarchy( dynamic_cast<ZBLogicalPrestationsEntity*>( pEntity ),
                                                      Level + 1 );
