...
{
    public:
        static std::string m_QuickSearch;
        const  char*       m_pKey1;
        const  char*       m_pKey2;
        ...
        * Constructor
        *@param pK1 - first key
        *@param pK2 - second key
        */
        kl(const char* pK1, const char* pK2) :
            m_pKey1(pK1),
            m_pKey2(pK2)
        {
            if (pK1)
                m_QuickSearch += m_pKey1;
        }

        /**
        * Converts at character
        *@param c - character
        */
        static int convertAt(char c)
        {
            char tmp[2];
            ...
            return m_QuickSearch.find(tmp);
        }
};
//---------------------------------------------------------------------------
std::string kl::m_QuickSearch = _T("");
//---------------------------------------------------------------------------
/**
* Mapping table
*/
...
// PSS_Iso8859
//---------------------------------------------------------------------------
std::string PSS_Iso8859::ANSItoISO8859(const std::string& str)
{
    ...
    {
        const int pos = kl::convertAt(str[i]);

        if (pos >= 0)
            result += dictionary[pos].m_pKey2;
        else
            result += str[i];
    }
...
//---------------------------------------------------------------------------
std::string PSS_Iso8859::ISO8859toANSI(const std::string& str)
{
    std::string result = str;
    int         pos    = 0;

    while (pos >= 0)
        for (int i = 0; dictionary[i].m_pKey1; ++i)
        {
            pos = result.find(dictionary[i].m_pKey2);

            if (pos >= 0)
            {
                result.replace(pos, int(std::strlen(dictionary[i].m_pKey2)), dictionary[i].m_pKey1);
                break;
            }
        }
